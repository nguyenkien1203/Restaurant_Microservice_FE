...
import { X, CalendarDays, Clock, Users, MapPin, Phone, Mail, User, ChevronDown, ChevronUp, Loader2, ShoppingBag } from 'lucide-react'
...
interface ReservationDetailsCardProps {
    ...
    onClose: () => void
    onStatusUpdate?: (reservationId: number, newStatus: ReservationStatus) => Promise<void>
    ...
    return (
        <Card className="sticky top-4 overflow-y-auto max-h-[calc(100vh-12rem)]">
            ...
            <CardContent className="space-y-6">
                ...
                <div className="space-y-3">
                    ...
                    <div className="grid gap-2">
                        ...
                        <div className="flex items-center gap-2 text-sm text-muted-foreground">
                            ...
                            <span>
                                {formatTime24to12(reservation.startTime)} - {formatTime24to12(reservation.endTime)}
                            </span>
                        </div>
                    </div>
                </div>
                            ...
                            <div className="flex items-center gap-2 text-sm">
                                ...
                                <span>
                                    ...
                                    {reservation.table.description && (
                                        <span className="text-muted-foreground"> - {reservation.table.description}</span>
                                    )}
                                </span>
                            </div>
                        )}
                ...
                {!reservation.userId && (reservation.guestName || reservation.guestEmail || reservation.guestPhone) && (
                    <div className="space-y-3">
                                ...
                                {isLoadingOrder ? (
                                    ...
                                    <>
                                        ...
                                        <div className="flex items-center justify-between">
                                            ...
                                            <span className={`text-xs px-2 py-0.5 rounded-full ${preOrder.status === 'COMPLETED' ? 'bg-green-100 text-green-700' :
                                                preOrder.status === 'PENDING' ? 'bg-yellow-100 text-yellow-700' :
                                                    preOrder.status === 'PREPARING' ? 'bg-blue-100 text-blue-700' :
